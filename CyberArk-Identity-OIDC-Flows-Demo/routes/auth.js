...
function Poll(
  ...
  restRequest(
    ...
    if (resp.Result.Summary == "OobPending" && Date.now() < endTime) {
      //OOB pending cases
      setTimeout(() => {
        data.Action = "Poll";
        Poll(
...
router.get("/RedirectToNextState", (req, res, next) => {
  ...
  if (state == "Error") {
    ...
    res.render("customerror", {
      ...
      error_description: req.session.ErrorMessage,
    });
  } else if (state == "NewPackage") {
    //new package)
...
function PrepareSession(req, package) {
  ...
  } else if (package.Result.Summary == "NewPackage") {
    //new package
...
router.post("/pollOOB", (req, res, next) => {
  Poll(
    req.body.TenantId,
    req.body.SessionId,
    req.body.MechanismId,
    req.body.Action,
    PollIntervalInMs,
    Date.now() + PollTimeOutInMs,
    function (resp, error) {
      if (error) {
        ...
        if (resp.success == true) {
          if (resp.Result.Auth) {
            res.send({
              success: true,
              result: resp.Result.Summary,
              Auth: resp.Result.Auth,
            });
          } else {
...
router.post("/choosemech", (req, res, next) => {
  if (req.body.mechanismId) {
    ...
    var mech = req.session.Mechanisms.find(
      (mech) => mech.MechanismId == mechId
    );
    if (mech) {
      ...
      res.render("customerror", {
        error: "Mechanism used does not belong to current session",
